...
import { loadApps, addApp as add, removeApp as remove } from '../lib/api';
/**
...
export function useApps() {
    ...
    useEffect(() => {
        const onStorage = (e) => {
            if (e.key === 'pingdev-apps')
                refresh();
        };
        const onCustom = () => refresh();
        window.addEventListener('storage', onStorage);
        window.addEventListener('pingdev-apps-changed', onCustom);
        return () => {
            window.removeEventListener('storage', onStorage);
            window.removeEventListener('pingdev-apps-changed', onCustom);
        };
    }, [refresh]);
    ...
    const removeApp = useCallback((port) => {
