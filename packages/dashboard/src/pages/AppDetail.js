...
import { fetchJob, fetchTools, sendChat, submitJob } from '../lib/api';
    ...
    return (<div className="page">
      ...
      <div className="detail-grid">
        <div className="col">
          ...
          <JobConsole port={app.port} tools={tools} onSignal={(lvl, msg, meta) => push({ level: lvl, kind: 'job', appName: app.name, appPort: app.port, message: msg, meta })} onToast={(intent, title, message) => toast({ intent, title, message })}/>
        </div>
...
function ToolShelf({ tools }) {
    return (<div className="panel">
      ...
      {tools.length === 0 ? (<div className="empty mini">
          ...
          {tools.map(t => (<div key={t.name} className="tool">
...
function JobConsole({ port, tools, onSignal, onToast, }) {
    ...
    useEffect(() => {
        ...
        fetchJob(port, activeJobId)
            ...
            onToast(intent, `Job ${r.status}`, activeJobId);
            onSignal(intent, `Job ${r.status}`, { job_id: activeJobId });
        })
            .catch(() => { });
    ...
    return (<div className="panel">
      ...
      <div className="console">
        <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Write a prompt. Cmd/Ctrl+Enter to run." onKeyDown={e => {
        ...
        <div className="console-row">
          <div className="pillrow">
            {hintedTools.map(t => (<button key={t.name} className={`pill ${tool === t.name ? 'on' : ''}`} onClick={() => setTool(prev => (prev === t.name ? '' : t.name))} title={t.description ?? t.name}>
                {t.name}
              </button>))}
          </div>
          <div className="console-actions">
            {connected ? (<button className="btn subtle" onClick={disconnect}>Stop Stream</button>) : (<span className="badge muted">SSE {activeJobId ? 'ready' : 'idle'}</span>)}
        ...
        {(events.length > 0 || activeJobId) && (<div className="stream">
            ...
            <div className="stream-box" ref={streamRef}>
              {events.map((evt, i) => (<div key={i} className="evt">
                  ...
                  <div className="evt-body">
                    ...
                    ? String(evt.data.text ?? '').slice(0, 400)
        ...
        {result && (<div className="result">
            <div className="result-head">
              ...
              <div className="result-meta">
                ...
                {result.timing?.total_ms != null && (<span className="badge info">{(result.timing.total_ms / 1000).toFixed(1)}s</span>)}
              </div>
            </div>
        ...
        {recent.length > 0 && (<div className="recent">
            ...
            <div className="recent-list">
              {recent.map(r => (<button key={r.jobId} className="recent-item" onClick={async () => {
