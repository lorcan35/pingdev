...
export function HealthPulse({ status, size = 12, }) {
    const st = statusStyles(status);
    return (<span className="relative inline-flex items-center justify-center" style={{ width: size, height: size }} title={st.label} aria-label={`health ${String(status)}`}>
      <span className={`absolute inset-0 rounded-full opacity-30 blur-[1px] ${st.ring} ${status === 'loading' ? '' : 'animate-pulseGlow'}`}/>
      <span className={`relative block h-full w-full rounded-full ${st.dot}`}/>
    </span>);
}
export function QueueFlow({ waiting, active, completed, failed, }) {
    ...
    const seg = (n) => `${Math.max(2, Math.round((n / total) * 100))}%`;
...
export function StateStrip({ waiting, active, }) {
    const w = Math.max(0, waiting);
    const a = Math.max(0, active);
...
//# sourceMappingURL=AppViz.js.map