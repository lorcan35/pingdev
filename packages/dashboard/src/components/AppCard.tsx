...
import type { PingAppConfig, HealthResponse } from '../lib/api';
import type { AppHealth } from '../hooks/useHealth';
import { HealthPulse, QueueFlow, StateStrip } from './AppViz';

function statusOf(st?: AppHealth | null): HealthResponse['status'] | 'offline' | 'loading' {
  if (!st) return 'loading';
  if (st.error) return 'offline';
...
export function AppCard({
  ...
  return (
    <div className={`appcard st-${status}`}>
      ...
      <div className="appcard-mid">
        ...
        <div className="appcard-statusline">
          ...
          {health?.health?.worker?.current_job && (
            <span className="badge info">job {String(health.health.worker.current_job).slice(0, 8)}…</span>
          )}
        </div>
      </div>
