...
const cdpUrl = getFlag('cdp-url');
const llmEndpoint = getFlag('llm-endpoint');
const llmModel = getFlag('llm-model');
async function main() {
    ...
    const result = await (0, pipeline_js_1.runRecon)({
        url: url,
        cdpUrl,
        outputDir,
        llmEndpoint,
    ...
    if (result.analysis) {
        ...
        (0, node_fs_1.writeFileSync)(analysisPath, JSON.stringify(result.analysis, null, 2));
...
//# sourceMappingURL=run-recon.js.map