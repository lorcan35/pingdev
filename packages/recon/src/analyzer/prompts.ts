...
function buildUserMessage(snapshot: SiteSnapshot, docs?: DocScrapeResult): string {
  ...
  for (const el of elements) {
    ...
    if (el.states.length) parts.push(`states=[${el.states.join(',')}]`);
    if (el.regionName) parts.push(`region="${el.regionName}"`);

    const selectors: string[] = [];
    if (el.ariaSelectors.length) selectors.push(el.ariaSelectors[0]);
    if (el.cssSelectors.length) selectors.push(el.cssSelectors[0]);
