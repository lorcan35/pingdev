...
class LLMClient {
    ...
    async anthropicChat(messages, options) {
        ...
        try {
            ...
            throw new Error(`Anthropic request failed (network): ${err.message}`);
        }
        if (!response.ok) {
    ...
    buildOpenAIBody(messages, options) {
        return {
            model: this.model,
            messages,
            temperature: options?.temperature ?? this.defaultTemperature,
    ...
    async postOpenAI(body) {
        ...
        try {
            ...
            throw new Error(`LLM request failed (network): ${err.message}`);
        }
        if (!response.ok) {
            const text = await response.text().catch(() => '');
