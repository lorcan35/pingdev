...
exports.generateActionFile = generateActionFile;
exports.generateActionsIndex = generateActionsIndex;
exports.generateMainIndex = generateMainIndex;
...
function generatePackageJson(name, url) {
    const pkg = {
        name: `@pingapps/${name}`,
        version: '0.1.0',
...
function generateStates(stateTransitions) {
    const lines = [
...
function generateActionFile(action, selectors) {
...
function generateActionsIndex(actions) {
    const lines = [
        "import type { ActionHandler } from '@pingdev/core';",
        '',
    ];
...
function generateMainIndex(name, url, actions) {
    const lines = [
    ...
    for (const action of actions) {
        const camelName = toCamel(action.name);
        if (allKnownSlots.has(camelName))
...
function generateTestFile(name, actions) {
    const lines = [
...
function generateReadme(siteDefinition) {
    const lines = [
