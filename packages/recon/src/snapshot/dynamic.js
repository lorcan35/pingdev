...
exports.detectDynamicAreas = detectDynamicAreas;
...
async function detectDynamicAreas(page, observeDurationMs = OBSERVE_DURATION_MS) {
    const areas = await page.evaluate((durationMs) => {
        return new Promise((resolve) => {
            ...
            const classifyDynamic = (el, types) => {
                const role = el.getAttribute('role');
                ...
                if (ariaLive === 'polite' || ariaLive === 'assertive')
                    return 'live-update';
                if (el.classList.contains('loading') || el.classList.contains('spinner'))
                    return 'loading-indicator';
                if (types.has('childList') && types.has('characterData'))
                    return 'response-output';
                if (types.has('childList'))
                    return 'live-update';
                return 'live-update';
            };
            const classifyStaticPattern = (el) => {
                const role = el.getAttribute('role');
                if (role === 'log')
                    return 'response-output';
                if (role === 'status')
                ...
                if (role === 'progressbar')
                    return 'loading-indicator';
                if (el.getAttribute('aria-busy') === 'true' || el.hasAttribute('data-loading'))
                    return 'loading-indicator';
                if (el.classList.contains('loading') || el.classList.contains('spinner'))
                    return 'loading-indicator';
                const ariaLive = el.getAttribute('aria-live');
            ...
            const observer = new MutationObserver((mutations) => {
                for (const mutation of mutations) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        if (container.id || container.getAttribute('role') || container.classList.length > 0)
                            break;
                    ...
                    const entry = mutationCounts.get(container) ?? { count: 0, types: new Set() };
                    entry.count++;
            ...
            setTimeout(() => {
                ...
                const liveAreas = document.querySelectorAll('[aria-live], [role="log"], [role="status"], [role="alert"], ' +
