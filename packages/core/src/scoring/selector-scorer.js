...
exports.scoreSelectorTier = scoreSelectorTier;
exports.scoreSelectorDef = scoreSelectorDef;
...
function nestingDepth(selector) {
...
function scoreSelectorTier(selector) {
    ...
    if (/\[role=/.test(s) || /\[aria-label=/.test(s) || /\[aria-/.test(s) || s.startsWith('role:')) {
        ...
        if (/\[aria-label=/.test(s) && /\[role=/.test(s))
            confidence = 1.0;
        return {
            name: s,
            tier: 'aria',
    ...
    if (s.startsWith('//') || s.startsWith('xpath=')) {
        ...
        if (/text\(\)/.test(s))
            confidence = 0.5;
        if (/\[contains\(/.test(s) && /text\(\)/.test(s))
            confidence = 0.45;
        if (/\[\d+\]/.test(s))
            confidence = 0.3; // positional xpath
        // Complex deep xpaths
...
function applyPenalties(selector, confidence) {
    let c = confidence;
    // Dynamic/auto-generated ID penalty
    if (isDynamicId(selector))
        c -= 0.3;
    // Deep nesting penalty
    if (nestingDepth(selector) > 3)
        c -= 0.2;
    // Long class chain penalty
    if (classCount(selector) > 3)
        c -= 0.15;
    return clamp(c);
}
...
function scoreSelectorDef(selectorDef) {
...
//# sourceMappingURL=selector-scorer.js.map