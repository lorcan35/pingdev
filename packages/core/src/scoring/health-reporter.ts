...
import { scoreAction } from './action-scorer.js';
...
export function generateHealthReport(
  ...
  const actionScores = selectorEntries.map(([key, selectorDef]) => {
    ...
    const action = scoreAction(key, [selectorDef], validationResult);

    // Analyze selector scores for warnings and recommendations
    const scores = scoreSelectorDef(selectorDef);
    ...
    const dynamicScore = scores.find(s =>
      s.reason.includes('auto-generated') || s.reason.includes('Auto-generated')
    );
    if (dynamicScore) {
  ...
  const overallScore = actionScores.length > 0
    ? Math.round(
        (actionScores.reduce((sum, a) => sum + a.overallScore, 0) / actionScores.length) * 100
      )
    : 0;
