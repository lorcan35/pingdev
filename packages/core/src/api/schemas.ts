...
export function buildSchemas(site: SiteDefinition): SiteSchemas {
  const toolSchema = site.tools && site.tools.length > 0
    ? { type: 'string' as const, enum: site.tools }
    : { type: 'string' as const };

  const modeSchema = site.modes && site.modes.length > 0
    ? { type: 'string' as const, enum: site.modes }
    : { type: 'string' as const };

  const substateSchema = site.substates && site.substates.length > 0
    ? { type: ['string', 'null'] as const, enum: [...site.substates, null] }
    : { type: ['string', 'null'] as const };

  const toolNullableSchema = { ...toolSchema, type: ['string', 'null'] as const };
  const modeNullableSchema = { ...modeSchema, type: ['string', 'null'] as const };
