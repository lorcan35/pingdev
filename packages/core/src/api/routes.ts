...
import type { SiteDefinition, JobRequest, EnhancedJobResult, HealthStatus, SSEEventType, RedisConfig, ShimAppOptions } from '../types.js';
import { BrowserAdapter } from '../browser/adapter.js';
import { RateLimiter } from './rate-limiter.js';
import { IdempotencyStore } from './idempotency.js';
import * as jobStore from '../worker/job-state-store.js';
...
export async function registerRoutes(
  ...
  const log = createLogger(site.name).child({ module: 'api' });
  const { queue, rateLimiter, idempotencyStore } = options;
  const schemas = buildSchemas(site);
  ...
  async function enforceRateLimits(): Promise<{ status: number; body: object } | null> {
  ...
  app.post<{ Body: JobRequest }>('/v1/jobs', { schema: schemas.postJobsSchema }, async (request, reply) => {
    ...
    const jobId = body.idempotency_key ?? randomUUID();
  ...
  app.get<{ Params: { id: string } }>('/v1/jobs/:id', { schema: schemas.getJobSchema }, async (request, reply) => {
    ...
    let result: EnhancedJobResult | null = null;
  ...
  /** GET /v1/jobs/:id/status — Real-time detailed status. */
  app.get<{ Params: { id: string } }>('/v1/jobs/:id/status', { schema: schemas.getJobStatusSchema }, async (request, reply) => {
    const { id } = request.params;
  ...
  /** GET /v1/jobs/:id/thinking — Extract thinking/reasoning content. */
  app.get<{ Params: { id: string } }>('/v1/jobs/:id/thinking', { schema: schemas.getJobThinkingSchema }, async (request, reply) => {
    const { id } = request.params;
    ...
    const liveState = jobStore.getJobState(id);
    const result = job.returnvalue as EnhancedJobResult | undefined;
  ...
  app.post<{ Body: JobRequest }>('/v1/chat', { schema: schemas.postChatSchema }, async (request, reply) => {
    const body = request.body;
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      if (state === 'completed') {
        ...
        const result = freshJob?.returnvalue as EnhancedJobResult | undefined;
        return reply.send({
          job_id: job.id,
  ...
  /** GET /v1/jobs/:id/stream — SSE stream of job events. */
  app.get<{ Params: { id: string } }>('/v1/jobs/:id/stream', { schema: schemas.getJobStreamSchema }, async (request, reply) => {
    const { id } = request.params;
    ...
    function sendEvent(type: SSEEventType, data: Record<string, unknown>): void {
  ...
  app.get('/v1/health', { schema: schemas.getHealthSchema }, async (_request, reply) => {
    ...
    try {
      const adapter = new BrowserAdapter(site.browser);
