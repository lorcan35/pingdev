...
export function buildDiscoverSummaryForLLM(snapshot, elementLimit = 30, maxChars = 5000) {
    const elements = (snapshot.elements || []).slice(0, elementLimit);
    const summary = elements.map((e) => `<${e.tag}${e.id ? ` id=\"${e.id}\"` : ''}${e.ariaLabel ? ` aria-label=\"${e.ariaLabel}\"` : ''}${e.selector ? ` selector=\"${e.selector}\"` : ''}>${(e.text || '').slice(0, 40)}</${e.tag}>`).join('\n');
    return truncateDom(summary, maxChars);
}
// Price patterns across currencies
const PRICE_RE = /(?:\$|€|£|¥|₹|USD|EUR|GBP)\s*[\d,.]+|[\d,.]+\s*(?:\$|€|£|¥|₹|USD|EUR|GBP)/i;
function classifyProduct(snapshot) {
    let score = 0;
    const fields = {};
    // Check JSON-LD for Product schema
    const productLd = snapshot.jsonLd?.find((ld) => ld['@type']?.toLowerCase() === 'product');
    if (productLd) {
        score += 0.4;
    }
    // Check for price patterns in text
    const priceElements = (snapshot.elements ?? []).filter((el) => PRICE_RE.test(el.text ?? ''));
    if (priceElements.length > 0) {
        score += 0.25;
        const best = priceElements.find((e) => e.selector) ?? priceElements[0];
        if (best.selector)
            fields.price = { selector: best.selector };
    }
    ...
    const nameEl = (snapshot.elements ?? []).find((el) => el.attributes?.['itemprop'] === 'name' ||
    ...
    const ratingEl = (snapshot.elements ?? []).find((el) => el.ariaLabel?.toLowerCase().includes('rating') ||
    ...
    // Product image
    const imgEl = (snapshot.elements ?? []).find((el) => el.tag === 'img' &&
...
function classifySearch(snapshot) {
    ...
    const searchInput = (snapshot.elements ?? []).find((el) => el.tag === 'input' &&
    ...
    // Pagination
    const pagination = (snapshot.elements ?? []).find((el) => el.ariaLabel?.toLowerCase().includes('pagination') ||
...
function classifyArticle(snapshot) {
    let score = 0;
    const fields = {};
    // JSON-LD Article type
    const articleLd = snapshot.jsonLd?.find((ld) => typeof ld['@type'] === 'string' &&
    ...
    // Byline / author
    const authorEl = (snapshot.elements ?? []).find((el) => el.attributes?.['itemprop'] === 'author' ||
    ...
    // Published date
    const dateEl = (snapshot.elements ?? []).find((el) => el.tag === 'time' ||
    ...
    // Title (h1)
    const h1 = (snapshot.elements ?? []).find((el) => el.tag === 'h1' && (el.text?.length ?? 0) > 10);
    if (h1?.selector) {
        score += 0.05;
        fields.title = { selector: h1.selector };
    }
...
function classifyFeed(snapshot) {
    let score = 0;
    const fields = {};
    ...
    const loadMore = (snapshot.elements ?? []).find((el) => (el.text ?? '').toLowerCase().includes('load more') ||
...
function classifyTable(snapshot) {
    if (!snapshot.tables || snapshot.tables.length === 0)
        return null;
    const largest = snapshot.tables.reduce((a, b) => a.rowCount > b.rowCount ? a : b);
    if (largest.rowCount < 2)
        return null;
    let score = 0.3;
    if (largest.headers.length > 0)
        score += 0.2;
    if (largest.rowCount >= 10)
        score += 0.15;
    if (largest.rowCount >= 50)
        score += 0.1;
    const fields = {
    ...
    const schemas = [{ name: 'table_data', fields }];
    return { pageType: 'table', confidence: Math.min(score, 0.99), schemas };
}
function classifyForm(snapshot) {
    if (!snapshot.forms || snapshot.forms.length === 0)
        return null;
    const form = snapshot.forms.reduce((a, b) => a.inputs.length > b.inputs.length ? a : b);
...
function classifyChat(snapshot) {
    let score = 0;
    const fields = {};
    // Look for chat-like patterns: textarea + send button + message list
    const textarea = (snapshot.elements ?? []).find((el) => el.tag === 'textarea' ||
    ...
    // Send button
    const sendBtn = (snapshot.elements ?? []).find((el) => el.tag === 'button' &&
    ...
    if (snapshot.repeatedGroups) {
        const msgGroup = snapshot.repeatedGroups.find((g) => g.count >= 2 && g.itemSelector.includes('message'));
        if (msgGroup) {
            score += 0.2;
 ...
 */
export function discoverPage(snapshot) {
    let best = { pageType: 'unknown', confidence: 0, schemas: [] };
...
//# sourceMappingURL=discover-engine.js.map