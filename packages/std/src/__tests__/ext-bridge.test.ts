...
describe('Device request/response via HTTP → WS → extension', () => {
  it('forwards HTTP POST /v1/dev/:device/:op to extension and returns result', async () => {
    ...
    ws.on('message', (buf) => {
      const msg = JSON.parse(buf.toString('utf-8')) as ExtDeviceRequest;
      if (msg.type === 'device_request') {
        const resp: ExtDeviceResponse = {
          type: 'device_response',
          id: msg.requestId,
          ok: true,
          result: { clicked: true, selector: (msg.command as any).selector },
        };
