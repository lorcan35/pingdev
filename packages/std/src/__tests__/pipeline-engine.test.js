...
describe('PipelineEngine', () => {
    ...
    describe('run', () => {
        ...
        it('resolves tab names by URL match', async () => {
            ...
            expect(result.steps[0].status).toBe('ok');
            expect(bridge.callDevice).toHaveBeenCalledWith(expect.objectContaining({ deviceId: 'tab-1' }));
        });
        ...
        it('normalizes read params from nested params object', async () => {
            ...
            const result = await engine.run({
                ...
                steps: [
                    { id: 's1', op: 'read', tab: 'tab-1', params: { selector: '.title' }, output: 'title' },
                ],
            });
            expect(result.steps[0].status).toBe('ok');
        ...
        it('normalizes read params from schema.selector fallback', async () => {
            ...
            const result = await engine.run({
                ...
                steps: [
                    { id: 's1', op: 'read', tab: 'tab-1', schema: { selector: '.price' }, output: 'price' },
                ],
            });
            expect(result.steps[0].status).toBe('ok');
            expect(bridge.callDevice).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('parsePipeShorthand', () => {
        it('parses extract|transform|type pipe', () => {
            const pipeline = PipelineEngine.parsePipeShorthand('extract:amazon:.price | transform:Deal: {{s1_result.value}} | type:slack:#msg');
...
//# sourceMappingURL=pipeline-engine.test.js.map