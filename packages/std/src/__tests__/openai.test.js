...
describe('OpenAIAdapter — health()', () => {
    it('returns online on successful 200 response', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ data: [{ id: 'gpt-4o' }] }, 200)));
        const adapter = createAdapter();
    ...
    it('returns offline with auth error message on 401', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ error: 'invalid_api_key' }, 401)));
        const adapter = createAdapter();
        const result = await adapter.health();
...
describe('OpenAIAdapter — execute()', () => {
    const baseRequest = { prompt: 'Hello, world!' };
...
describe('OpenAIAdapter — listModels()', () => {
    ...
    it('falls back to static list on non-OK HTTP response', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(textResponse('Service Unavailable', 503)));
        const adapter = createAdapter();
