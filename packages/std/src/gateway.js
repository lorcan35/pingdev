...
import { learnTemplate, applyTemplate, findTemplateForUrl, loadTemplate, listTemplates, deleteTemplate, exportTemplate, importTemplate, } from './template-learner.js';
import { cdpFallback } from './cdp-fallback.js';
const CRASH_LOG_PATH = '/tmp/pingos-crash.log';
let crashHandlersInstalled = false;
function appendCrashLog(event, err) {
...
export async function createGateway(opts) {
    ...
    app.get('/v1/devices', async () => {
        ...
        const devices = clients.flatMap(({ clientId, tabs }) => (tabs ?? []).map((t) => ({ ...t, clientId })));
        return {
            extension: {
                clients,
                devices,
    ...
    app.get('/v1/dev/:device/discover', async (request, reply) => {
        const { device } = request.params;
        ...
        try {
            ...
            const result = discoverPage(snapshot);
    ...
    app.post('/v1/dev/:device/discover', async (request, reply) => {
        const { device } = request.params;
        if (!extBridge.ownsDevice(device)) {
            return reply.status(404).send({
                errno: 'ENODEV',
    ...
    app.post('/v1/dev/:device/suggest', async (request, reply) => {
        const { device } = request.params;
    ...
    const diffStorage = new Map();
    function featureHash(str) {
        let h = 0;
    ...
    async function getDeviceDom(deviceId, maxChars = 15_000) {
        ...
        try {
            ...
            if (typeof result === 'string')
                return truncateDom(result, cap);
            if (result && typeof result === 'object') {
                const html = result.html ?? result.data ?? result;
    ...
    async function extractSchemaFromDevice(deviceId, schema) {
        try {
    ...
    function extractExplicitCountFromQuery(query) {
        const q = (query ?? '').trim();
        if (!q)
            return null;
        const patterns = [
    ...
    function applyExplicitQueryCountLimit(data, query) {
    ...
    async function callFeatureLLM(prompt, opts) {
        const local = isLocalMode();
    ...
    function featureParseJson(text) {
        try {
    ...
    app.post('/v1/dev/:device/query', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
        if (!body || !body.question) {
            return reply.status(400).send({
    ...
    app.post('/v1/dev/:device/watch', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
    ...
    app.post('/v1/dev/:device/diff', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
        if (!body || !body.schema || typeof body.schema !== 'object') {
            return reply.status(400).send({
    ...
    app.post('/v1/apps/generate', async (request, reply) => {
        const body = request.body;
        ...
        try {
            ...
            if (local) {
                ...
                return { app, model: getLocalConfig().llmModel || 'local' };
            }
    ...
    app.get('/v1/functions/:app', async (request, reply) => {
    ...
    app.post('/v1/functions/:app/call', async (request, reply) => {
        const { app: appName } = request.params;
        const body = request.body;
    ...
    app.post('/v1/functions/:app/batch', async (request, reply) => {
        const { app: appName } = request.params;
        const body = request.body;
        ...
        try {
            const results = await funcRegistry.batch(body.calls.map((c) => ({
    ...
    app.post('/v1/dev/:device/watch/start', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
    ...
    app.get('/v1/watches/:watchId/events', async (request, reply) => {
    ...
    app.post('/v1/pipelines/run', async (request, reply) => {
        const body = request.body;
        if (!body || !body.steps) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/pipelines/validate', async (request, reply) => {
        const body = request.body;
        if (!body) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/pipelines/save', async (request, reply) => {
        const body = request.body;
    ...
    app.post('/v1/pipelines/pipe', async (request, reply) => {
        const body = request.body;
        if (!body || !body.pipe) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/dev/:device/:op', async (request, reply) => {
        ...
        if (extBridge.ownsDevice(device)) {
            ...
            if (op === 'extract' && !payloadObj?.paginate && !payloadObj?.strategy) {
                ...
                if (deviceUrl) {
                    ...
                    if (template) {
                        ...
                        const schemaIsEmpty = !payloadObj?.schema || Object.keys(payloadObj.schema).length === 0;
                        if (schemaIsEmpty) {
                            try {
                                const tmplResult = await applyTemplate(extBridge, device, template);
                                const limitedTemplateData = applyExplicitQueryCountLimit(tmplResult.data, payloadObj?.query);
            ...
            if (op === 'extract' && payloadObj?.paginate) {
                try {
                    const result = await paginateExtract(extBridge, {
                        deviceId: device,
                        schema: payloadObj.schema,
                        query: payloadObj.query,
                        paginate: true,
                        maxPages: payloadObj.maxPages ?? 10,
                        delay: payloadObj.delay ?? 1000,
                    });
            ...
            if (op === 'wait' || op === 'waitFor') {
                if (payloadObj && !payloadObj.timeout)
                    payloadObj.timeout = 10_000;
                if (payloadObj && !payloadObj.timeoutMs)
                    payloadObj.timeoutMs = 10_000;
            }
            try {
                ...
                let result;
                if (op === 'wait' || op === 'waitFor') {
                    const serverTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Server-side wait timeout (15s)')), 15_000));
                    try {
                ...
                if (op === 'extract' && payloadObj?.fallback === 'visual') {
                    ...
                    const isEmpty = !resData || Object.values(resData).every(v => v === '' || v === null || (Array.isArray(v) && v.length === 0));
                    if (isEmpty) {
                ...
                const isEIO = isPingError(err)
                    ? err.errno === 'EIO'
                    : (err instanceof Error && (err.message.includes('I/O error') || err.message.includes('EIO') ||
    ...
    const semanticSelectorCache = new Map();
    const SEMANTIC_CACHE_TTL_MS = 30 * 60 * 1000; // 30 minutes
    app.post('/v1/dev/:device/extract/semantic', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
        if (!body || !body.query) {
            return reply.status(400).send({
        ...
        try {
            ...
            const elemSummary = buildDiscoverSummaryForLLM(snapshotObj, envInt('PINGOS_LLM_EXTRACT_ELEMENT_LIMIT', 30), local ? localCfg.domLimit : envInt('PINGOS_LLM_EXTRACT_DOM_MAX_CHARS', 5_000));
            ...
            try {
                ...
                selectors = Object.fromEntries(Object.entries(parsed || {}).filter(([, v]) => typeof v === 'string'));
            }
            catch {
            ...
            const resultData = resultObj?.result || resultObj || {};
    ...
    app.post('/v1/dev/:device/extract/learn', async (request, reply) => {
        const { device } = request.params;
        const body = request.body;
        if (!body || !body.schema) {
            return reply.status(400).send({
        ...
        try {
            ...
            const template = await learnTemplate(extBridge, device, extractResult, body.schema);
    ...
    app.get('/v1/templates/:domain', async (request, reply) => {
    ...
    app.delete('/v1/templates/:domain', async (request, reply) => {
    ...
    app.post('/v1/templates/import', async (request, reply) => {
        const body = request.body;
    ...
    app.get('/v1/templates/:domain/export', async (request, reply) => {
    ...
    // ---- Recording endpoints ----
    app.post('/v1/record/start', async (request, reply) => {
        const body = request.body;
        if (!body || !body.device) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/record/stop', async (request, reply) => {
        const body = request.body;
        if (!body || !body.device) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/record/export', async (request, reply) => {
        const body = request.body;
        if (!body || !body.device) {
            return reply.status(400).send({
                errno: 'ENOSYS',
        ...
        try {
            ...
            const extExport = extResult ?? {};
            const extSteps = (extExport.steps ?? extExport.actions ?? []);
    ...
    app.get('/v1/record/status', async (request, reply) => {
    ...
    const gatewayRecordings = new Map();
    function logRecordedAction(deviceId, op, payload, result, durationMs) {
        const rec = gatewayRecordings.get(deviceId);
        if (!rec)
            return;
        // Don't record internal recording ops
        if (op.startsWith('record_'))
            return;
        const payloadObj = payload && typeof payload === 'object' ? payload : undefined;
        const action = {
            type: op,
            ...
            value: payloadObj?.to ?? payloadObj?.text ?? payloadObj?.query ?? payloadObj?.url ?? undefined,
        };
    ...
    app.post('/v1/record/replay', async (request, reply) => {
        const body = request.body;
        if (!body || !body.device) {
            return reply.status(400).send({
                errno: 'ENOSYS',
        ...
        if (recording && !recording.actions && recording.result?.actions) {
            recording = { ...recording.result, id: recording.result?.id ?? 'inline' };
        }
    ...
    app.post('/v1/recordings/replay', async (request, reply) => {
        const body = request.body;
        if (!body || !body.device) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/recordings/generate', async (request, reply) => {
        const body = request.body;
        if (!body) {
            return reply.status(400).send({
                errno: 'ENOSYS',
    ...
    app.post('/v1/recordings/save', async (request, reply) => {
        const body = request.body;
    ...
    app.delete('/v1/recordings/:id', async (request, reply) => {
...
function isElementNotFoundError(err) {
    ...
    return (text.includes('element not found:') ||
