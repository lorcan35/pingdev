...
export class FunctionRegistry {
    ...
    refresh() {
        ...
        for (const { tabs } of tabClients) {
            for (const tab of tabs ?? []) {
                ...
                const pingApp = this.pingAppDefs.find((def) => tab.url?.toLowerCase().includes(def.domain));
                if (pingApp) {
     ...
     */
    async call(qualifiedName, params) {
        this.refresh();
        ...
        const fn = app.functions.find((f) => f.name === qualifiedName);
        if (!fn) {
            throw new Error(`Function "${qualifiedName}" not found. Available: ${app.functions.map((f) => f.name).join(', ')}`);
        }
    ...
    buildFunctions(deviceId, appName) {
