...
async function waitForAliExpressPrices(gateway, deviceId, timeoutMs = 12_000) {
    try {
...
async function extractWithRetries(gateway, deviceId, expression, hasData, retries = 2, retryDelayMs = 1500) {
...
async function fetchJsonWithTimeout(url, init, timeoutMs = ROUTE_TIMEOUT_MS) {
...
async function deviceOp(gateway, deviceId, op, payload = {}) {
    try {
        return await fetchJsonWithTimeout(`${gateway}/v1/dev/${deviceId}/${op}`, {
            method: 'POST',
...
async function findDeviceByDomain(gateway, domain) {
    try {
        const data = await fetchJsonWithTimeout(`${gateway}/v1/devices`, undefined, ROUTE_TIMEOUT_MS);
        const devices = data?.extension?.devices || [];
        const match = devices.find((d) => d.url?.includes(domain));
...
async function findDevice(gateway) {
    return findDeviceByDomain(gateway, 'aliexpress');
}
async function setAliExpressLocale(gateway, deviceId) {
 ...
 */
export const PINGAPP_FUNCTION_DEFS = [
    {
        app: 'aliexpress',
        domain: 'aliexpress',
        functions: [
...
export function registerAppRoutes(app, gatewayUrl) {
    ...
    app.post('/v1/app/aliexpress/search', async (req, reply) => {
        ...
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No AliExpress tab open' });
        // Navigate to search
        ...
        const result = await extractWithRetries(gatewayUrl, deviceId, EXTRACTORS.searchResults, (products) => Array.isArray(products) && products.some((p) => typeof p?.price === 'string' && p.price.trim().length > 0));
    ...
    app.post('/v1/app/aliexpress/product', async (req, reply) => {
        const { id } = req.body;
        if (!id)
            return reply.code(400).send({ ok: false, error: 'product id required' });
        const deviceId = await findDevice(gatewayUrl);
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No AliExpress tab open' });
        await setAliExpressLocale(gatewayUrl, deviceId);
        ...
        await waitForAliExpressPrices(gatewayUrl, deviceId);
        await deviceOp(gatewayUrl, deviceId, 'clean', { mode: 'full' });
        const result = await extractWithRetries(gatewayUrl, deviceId, EXTRACTORS.productDetails, (product) => Boolean(product && typeof product.price === 'string' && product.price.trim().length > 0));
    ...
    // AMAZON PingApp
    // =============================================
    function findAmazonDevice(gateway) {
        return findDeviceByDomain(gateway, 'amazon');
    }
    function findClaudeDevice(gateway) {
    ...
    app.post('/v1/app/amazon/search', async (req, reply) => {
        ...
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No Amazon tab open' });
    ...
    app.post('/v1/app/amazon/product', async (req, reply) => {
        const { asin } = req.body;
        if (!asin)
            return reply.code(400).send({ ok: false, error: 'asin required' });
        const deviceId = await findAmazonDevice(gatewayUrl);
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No Amazon tab open' });
    ...
    app.post('/v1/app/claude/chat', async (req, reply) => {
        ...
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No Claude tab open' });
    ...
    app.post('/v1/app/claude/model', async (req, reply) => {
        ...
        if (!model)
            return reply.code(400).send({ ok: false, error: 'model required' });
        const deviceId = await findClaudeDevice(gatewayUrl);
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No Claude tab open' });
    ...
    app.get('/v1/app/claude/projects', async (req, reply) => {
        ...
        if (!nav?.ok)
            await deviceOp(gatewayUrl, deviceId, 'eval', { expression: `window.location.href = "https://claude.ai/projects"` });
        await delay(2000);
    ...
    app.get('/v1/app/claude/artifacts', async (req, reply) => {
        ...
        if (!nav?.ok)
            await deviceOp(gatewayUrl, deviceId, 'eval', { expression: `window.location.href = "https://claude.ai/artifacts"` });
        await delay(2000);
    ...
    app.post('/v1/app/claude/upload', async (req, reply) => {
        ...
        if (!filePath)
            return reply.code(400).send({ ok: false, error: 'filePath required' });
        const deviceId = await findClaudeDevice(gatewayUrl);
        if (!deviceId)
            return reply.code(404).send({ ok: false, error: 'No Claude tab open' });
