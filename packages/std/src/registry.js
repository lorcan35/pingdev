...
import { resolveStrategy } from './routing/index.js';
export class ModelRegistry {
    drivers = new Map();
    healthCache = new Map();
    affinityMap = new Map(); // affinity key → driver id
    routingState = { counter: 0 };
    healthTimer;
    defaultStrategy;
    constructor(defaultStrategy = 'best') {
    ...
    resolve(request) {
        ...
        if (request.driver) {
            ...
            if (!driver)
                throw ENOENT(request.driver);
            return driver;
        }
        // 2. Build candidate list — filter by required capabilities
        let candidates = Array.from(this.drivers.values());
        if (request.require) {
            candidates = candidates.filter((d) => matchesCapabilities(d.registration.capabilities, request.require));
        }
        ...
        const picked = resolveStrategy(strategy, candidates, this.healthCache, this.routingState);
    ...
    findByCapability(cap) {
        return Array.from(this.drivers.values()).filter((d) => d.registration.capabilities[cap]);
    }
    ...
    // ---- internals ----
    async pollAll() {
        const tasks = Array.from(this.drivers.entries()).map(async ([id, driver]) => {
            try {
...
function matchesCapabilities(has, needs) {
    for (const [key, value] of Object.entries(needs)) {
        const k = key;
        if (value === true && !has[k])
            return false;
        if (typeof value === 'number' && has[k] < value)
            return false;
    }
    return true;
}
