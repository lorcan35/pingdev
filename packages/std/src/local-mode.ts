...
function normalizeBaseUrl(url: string): string {
  ...
  if (trimmed.includes('/v1/')) return trimmed.replace(/\/$/, '');
  return `${trimmed}/v1`;
}

export function getLocalConfig(): LocalModeConfig {
...
export function getTimeoutForFeature(feature: string): number {
  const cfg = getLocalConfig();
  const key = (feature || 'default').toLowerCase();
  if (!cfg.enabled) {
    return envInt('PINGOS_LLM_TIMEOUT_MS', 15_000);
  }
  if (key === 'query') return cfg.timeouts.query;
  if (key === 'heal') return cfg.timeouts.heal;
  if (key === 'generate') return cfg.timeouts.generate;
  if (key === 'suggest') return cfg.timeouts.suggest;
  if (key === 'extract' || key === 'paginate') return cfg.timeouts.extract;
  if (key === 'discover') return cfg.timeouts.discover;
  if (key === 'visual' || key === 'vision') return cfg.timeouts.visual;
...
export function getModelForFeature(feature: string): string {
  ...
  if ((key === 'extract' || key === 'paginate') && cfg.models.extract) return cfg.models.extract;
  if (key === 'heal' && cfg.models.heal) return cfg.models.heal;
  if (key === 'generate' && cfg.models.generate) return cfg.models.generate;
  if ((key === 'visual' || key === 'vision') && cfg.models.vision) return cfg.models.vision;
  if ((key === 'visual' || key === 'vision') && cfg.visionModel) return cfg.visionModel;
  return cfg.llmModel || 'default';
}

export function truncateDom(html: string, limit?: number): string {
