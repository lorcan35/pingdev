...
function lookup(ref: string, variables: Variables): unknown {
  ...
  for (const part of parts) {
    if (current == null) throw new Error(`Cannot access '${part}' of null/undefined`);
    ...
    if (part === 'length' && typeof current === 'object') {
      return Object.keys(current as object).length;
    }
...
export function resolveTemplate(text: string, variables: Variables): string {
  return text.replace(/\{\{(.+?)\}\}/g, (match, ref: string) => {
    ref = ref.trim();
    try {
...
export function resolveValue(text: string, variables: Variables): unknown {
...
const COMPARE_OPS: Record<string, (a: unknown, b: unknown) => boolean> = {
...
function resolveToken(token: string, variables: Variables): unknown {
  token = token.trim();
  const m = token.match(/^\{\{(.+?)\}\}$/);
  if (m) {
    const ref = m[1].trim();
...
function evalSimple(condition: string, variables: Variables): boolean {
...
function splitLogical(condition: string, sep: string): string[] {
  const parts: string[] = [];
  let depth = 0;
  let current = '';
  let i = 0;
...
export function evaluateCondition(condition: string, variables: Variables): boolean {
  condition = condition.trim();
...
export class WorkflowEngine {
  ...
  private totalRetriesUsed = 0;
  private executor: OpExecutor;
  ...
  async run(steps: WorkflowStep[]): Promise<WorkflowResult> {
  ...
  private resolveStep(step: WorkflowStep): Record<string, unknown> {
  ...
  private async runSteps(steps: WorkflowStep[]): Promise<{ lastResult: unknown; aborted?: boolean }> {
    ...
    for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
      ...
      if (op === 'if') {
        ...
        if (branch.length > 0) {
          ...
          if (sub.aborted) return { lastResult, aborted: true };
        }
        this.results.push({
          step,
      ...
      if (op === 'set') {
        ...
        this.variables[varName] = resolveValue(step.value as string, this.variables);
      ...
      try {
        ...
        const recovery = await this.handleErrorRecovery(
          step, stepIndex, err instanceof Error ? err : new Error(String(err)),
        );
  ...
  private async handleErrorRecovery(
    ...
    const onError = step.onError || (this.defaults.onError as WorkflowStep['onError']) || 'abort';
