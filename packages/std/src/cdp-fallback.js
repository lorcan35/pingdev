...
async function findCDPTarget(deviceId) {
...
async function findCDPTargetByUrl(url) {
    try {
        const res = await fetch(`http://${CDP_HOST}:${CDP_PORT}/json/list`);
        if (!res.ok)
            return null;
...
function buildExtractExpression(payload) {
...
function buildDiscoverExpression() {
    return `(function() {
...
function buildAssertExpression(assertions) {
...
function buildSemanticExpression(query) {
    return `(function() {
 ...
 */
export async function cdpFallback(deviceUrl, op, payload) {
    if (!deviceUrl)
        return null;
    ...
    switch (op) {
        ...
        case 'table':
            ...
            break;
        case 'assert':
            expression = buildAssertExpression(payload?.assertions ?? []);
            break;
        case 'recon':
            expression = `(function() {
        const actions = [];
    ...
    if (op === 'extract' && payload?.query) {
        expression = buildSemanticExpression(payload.query);
    }
    try {
...
//# sourceMappingURL=cdp-fallback.js.map